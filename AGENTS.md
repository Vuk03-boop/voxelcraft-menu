# AGENTS.md — Canonical Agent Rules & Project Contract
...
## 5. Agent Struggle & Fumble Log [AGENT-FUMBLE]
To prevent catastrophic remembering and repeated traps, respect documented failure modes:
- `[AGENT-FUMBLE: LINE-ENDINGS]` `Cargo.toml` and `.gitattributes` require CRLF; `.rs` and `.wgsl` require LF. Do not introduce mixed line endings.
- `[AGENT-FUMBLE: WGSL-ALIGNMENT]` Uniform buffers in WGSL enforce 16-byte alignment. Do not reorder fields in `GpuFrame` without padding.
- `[AGENT-FUMBLE: WATER-MOTTLE]` Do not attempt to re-enable `--water-mottle` in the pipeline; WA01 intentionally overrides it to `0.0`.
- `[AGENT-FUMBLE: RAY-PACKING]` Ray marching 64-bit visibility keys are densely packed; do not insert bitfields without updating unpack shaders.
